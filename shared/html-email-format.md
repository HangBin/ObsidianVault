...
当子内容本身也是 `- **加粗**` 格式时，预处理脚本需要：
1. 保持 `- ` 和 `**` 之间的空格（不要被正则吃掉）
2. 缩进到正确位置（content_col + 4 空格）

```markdown
# 原始MD
1. **三星罢工**
   - 三星电子工会...
   - **对A股影响**: xxx    ← 这是列表项内的子列表项
   - **对持仓影响**: xxx    ← 同上

# 预处理后（正确）
1. **三星罢工**
       - 三星电子工会...
       - **对A股影响**: xxx  ← 6空格缩进，- 和 ** 之间有空格
       - **对持仓影响**: xxx  ← 同上
```
...
| 加粗行后普通文本未转列表 | `**加粗**：` 后紧跟普通文本 | 步骤B：检测后插入空行+转为 `- ` 列表项 |
| `- **` 空格被正则吃掉 | `re.sub(r'([^\n]) +(\*\*)', ...)` 误匹配 `- **` | 排除 `-` 和 `*` 字符：`re.sub(r'([^\s*-]) +(\*\*)', ...)` |
...
## 七、2026-05-22 排版修复新增经验

### Q1：市场风向节 — 有序列表+无序列表混排

**问题**：`1. **事件**` 下紧跟 `- 子项` 无序列表，Python markdown 无法识别为嵌套列表。

**根因**：
1. emoji删除后 `1. **` 格式被破坏（变成 `1.**`）
2. 基础格式修复正则 `re.sub(r'([^\n]) +(\*\*)', ...)` 误吃掉 `- **` 中的空格
3. 子列表缩进不足4空格
...
# 3. 子列表项缩进 content_col + 4 空格
```

### Q2：行业政策节 — 加粗子列表项排版

**问题**：`- **持仓关联**: xxx` 和 `- **对A股影响**: xxx` 中的加粗未被正确渲染。

**根因**：`- **` 被正则变成 `-**`，Python markdown 无法识别为列表项内的加粗。

**修复方案**：
```python
# 在步骤A子列表项处理时，确保 - 和 ** 之间有空格
nl_content = re.sub(r'^([-*])\*\*', r'\1 **', nl_content)
```

### Q3：资金面分析节 — 加粗行后紧跟列表

**问题**：`**昨日主力资金流向**（来源：东方财富）：` 后紧跟 `- 全天净流出xxx`，被渲染为同一段落。

**修复方案**：步骤B检测加粗行以冒号结尾，插入空行：
```python
if re.search(r'\*\*[^*]+\*\*', line) and line.strip().endswith(('：', ':')):
...
**问题**：每个持仓下的"早盘预判/上午实际/午间更新/止盈线"子列表渲染不正确。

**修复方案**：
- 子列表项缩进：`child_indent = ' ' * (content_col + 4)`
- 嵌套子列表同样适用4空格缩进规则
- 最终结构：`<ul><li>父项<ul><li>子项</li></ul></li></ul>`
...
def _fix_bq_lists(m):
    inner = m.group(1)
    # 将 <p>操作原则：\n1. xxx\n2. xxx</p> 转为 <p>操作原则：</p><ol><li>xxx</li></ol>
```

### Q6：报告生成时间 — 连续斜体行

**问题**：三行 `*...*` 被合并到一个 `<p>` 里，第三行被识别为无序列表。

**修复方案**：
1. 每行之间插入空行
2. 第三行改用下划线斜体 `_..._` 避免被识别为列表

### Q7：临时文件清理规则

**问题**：脚本迭代产生 `/tmp/morning-email-v*.html`、`/tmp/*-debug.md` 等临时文件堆积。

**规则**：任务完成后立即清理 `/tmp/` 下的中间迭代文件，只保留最终版本。
```bash
rm /tmp/morning-email-v*.html /tmp/*-debug.md /tmp/test-*.html
```
