...
### 14.1 核心原则：先检查登录态，再发布

> **铁律：每次发布前，先检查 cookies 是否有效。失效就发二维码让用户扫码，不做多余分析。**

**标准流程（不可跳步）：**
```
1. bash /home/bill/run.sh --check          ← 先检查登录态
2a. 如果 "✅ 登录态正常" → 直接 run.sh 发布
2b. 如果 "❌ 需要登录" → 截图二维码 → message 发给用户 → 等用户扫码 → 再发布
3. 不做多余分析，不自己写 CDP 脚本
```
...
| 场景 | 正确做法 | 错误做法 |
|------|---------|----------|
| 发布商品 | 先 `run.sh --check`，通过后再 `run.sh --image ...` | ❌ 跳过登录态检查直接发布 |
| 上传图片 | run.sh 内部的 xianyu_publish.py 已包含完整流程 | ❌ 重新分析 fiber 树遍历 |
| 登录态过期 | 截图二维码 → message 发给用户扫码 | ❌ 自己写代码检测/分析登录态 |
| 截图给用户扫码 | 用 `message` 工具发图（见 14.3） | ❌ 自己写截图+OCR+发送 |
| cookies 失效 | 发二维码让用户重新登录 | ❌ 尝试自己刷新 cookies |
